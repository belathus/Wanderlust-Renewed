...
#  val eb_s_master = Functions.parse({"function": "ebwizardry:random_spell", "elements":["sorcery"], "tiers":["master"]} as crafttweaker.data.IData);
  ...
  val tf_hill_1 = LootTables.getTable("twilightforest:structures/hill_1/hill_1");

  ######################################
  ### Twilight Forest: Hollow Hill 1 ###
  ######################################
/*
    Modify the twilight forest small hollow hill table to contain...
*/
  tf_hill_1.clear();

  val tf_common = tf_hill_1.addPool("common", 3, 7, 0, 0);
...
#  tf_common.addItemEntryHelper(<ebwizardry:scroll>,          30, 0, [eb_s_any], []);
  ...
  val tf_uncommon = tf_hill_1.addPool("uncommon", 1, 3, 0, 0);
...
#  tf_uncommon.addItemEntryHelper(<ebwizardry:spell_book>,       90, 0, [eb_s_apprentice], []);
  ...
  tf_rare.addItemEntryHelper(<pyrotech:flint_pickaxe>, 1, 0, [tool_dam, tool_ench], []);
  tf_rare.addItemEntryHelper(<pyrotech:flint_shovel>, 1, 0, [tool_dam, tool_ench], []);
  ...
  val tf_ultrarare = tf_hill_1.addPool("ultrarare", 1, 1, 0, 0);
...
#  tf_ultrarare.addItemEntryHelper(<ebwizardry:spell_book>,         1, 0, [eb_s_advanced], []);
  ...
  tf_hill_1.addPool("forestry_bag", 1, 1, 0, 0).addItemEntryHelper(<forestry:adventurer_bag>, 1, 0, [], [Conditions.randomChance(0.05)]);
  tf_hill_1.addPool("forestry_bee", 1, 1, 0, 0).addItemEntryHelper(<forestry:bee_drone_ge>, 1, 0, [steadfast], [Conditions.randomChance(0.10)]);
  tf_hill_1.addPool("blood", 1, 1, 0, 0).addItemEntryHelper(<evilcraft:condensed_blood>, 1, 0, [Functions.setCount(1, 32)], [Conditions.randomChance(0.25)]);
  tf_hill_1.addPool("closure_box", 1, 1, 0, 0).addLootTableEntry("evilcraft:inject/chests/box_of_eternal_closure", 1);
  tf_hill_1.addPool("overgrowth", 1, 1, 0, 0).addItemEntryHelper(<botania:overgrowthseed>, 1, 0, [], [Conditions.randomChance(0.05)]);
  tf_hill_1.addPool("lotus", 1, 1, 0, 0).addItemEntryHelper(<botania:blacklotus>, 1, 0, [], [Conditions.randomChance(0.35)]);
  tf_hill_1.addPool("tome", 1, 1, 0, 0).addItemEntryHelper(<quark:ancient_tome>, 1, 0, [ancient_tome], [Conditions.randomChance(0.05)]);
